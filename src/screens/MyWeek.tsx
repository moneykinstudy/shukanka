  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // profiles.id を取得（auth.id / profiles.auth_user_id の両対応）
        ...
        const userIds = Array.from(new Set([me.id, profId].filter(Boolean))) as string[];
        ...
        const { data, error } = await supabase
          .from('study_logs')
          .select('study_date, subject, minutes, memo, user_id')
          .in('user_id', userIds)
          .gte('study_date', from)
          .lte('study_date', to);
        ...
        (data as Row[]).forEach(r => {
          ...
          if (freeNote) map[d].memo = freeNote;
        });
        ...
        setMemoPick(candidates.length > 0 ? candidates[Math.floor(Math.random() * candidates.length)] : null);
      } catch (e) {
