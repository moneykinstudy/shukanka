...
export function OnboardOverlay() {
  ...
  // 初期チェック
  useEffect(() => { tryOpenFromTrigger(); tryResumeStep2(); }, [tryOpenFromTrigger, tryResumeStep2]);

  // ナビ依存なしの再チェック
  useEffect(() => {
    const recheck = () => { tryOpenFromTrigger(); tryResumeStep2(); };
  ...
  }, [tryOpenFromTrigger, tryResumeStep2]);
        ...
        {step === 2 && (
          <>
            <View style={{ marginBottom: 12 }}>
              <Text style={{ fontSize: 16, fontWeight: '700', marginBottom: 4 }}>（任意）ホーム画面に追加</Text>
